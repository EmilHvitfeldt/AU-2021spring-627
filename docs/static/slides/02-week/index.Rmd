...
This allows to quantify the uncertainty
...
If $f$ is different then the null-model or monkey model.
...
This holds a major assumption that scenario in which we estimate $f$ stays the same.
...
Any error left when when a perfect statistical model is trained is the irreducible error  
...
unsupervised learning on the other hand doesn't have a explicit goal or answer sheet
...
> "here is all our costumer data, do they form groups?"
...
We are minimizing residual sum of squares (RSS)
...
Since this model in build on certain assumptions, we are able to calculate standard error estimates for each parameter estimate.

These standard errors can be used to determine if the estimates are significantly different from 0

Inverse relationship between size of effect and number of observations

---

# Assessing model accuarcy
...
Remember how *residuals squared sum* (RSS) depended on the number of of observations?
...
> RSE is the average amounnt that the response will deviate from the true regression line
...
> Is there an assosiation between Y and X

but when $p>1$ then question becomes

> Is at least one of the predictors $X_1, X_2, ..., X_p$ usefull in predicting the response

and 

> Which of the X's have an assosiation with Y
...
Linear model works well in a lot of cases.
...
- Remove data if if they are wrongly collected
- Consider using a model that isn't affected much by outliers

domain, linear models perform bad if some of the observations are FAR away from the other points
